    ...
    private fun dumpNfcIntent(source: String, intent: Intent) {
        ...
        for (key in extras.keySet()) {
            try {
                ...
                if (value is Array<*>) {
                    Log.d(TAG, "Extra[$key] is Array[] length=${value.size}")
                    for (i in value.indices) {
                        ...
                        } else if (p is Parcelable) {
                            Log.d(TAG, "  Parcelable[$i]: ${p?.javaClass?.simpleName} -> $p")
                        } else {
                            Log.d(TAG, "  Array element[$i]: ${p?.javaClass?.simpleName} -> $p")
                        }
                    }
                } else if (value is NdefMessage) {
                    Log.d(TAG, "Extra[$key] is NdefMessage records=${value.records.size}")
                } else {
                    Log.d(TAG, "Extra[$key] (${value?.javaClass?.simpleName}) -> $value")
                }
            } catch (e: Exception) {
